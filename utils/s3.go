...
func (s3Service *S3Service) UploadFile(file string, contentType string) (string, error) {
	s3Config := s3Service.S3Config
	info, err := s3Service.s3Client.FPutObject(s3Service.ctx, s3Config.BucketName, s3Config.FileLocation, file, minio.PutObjectOptions{ContentType: contentType})
